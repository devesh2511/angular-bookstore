...
export class AppComponent implements OnInit{
  ...
  displayedBooks:Array<Book> = this.books;
  ...
  addToCart(book:Book){
    ...
    this.totalAmount += +book.price.value;
  ...
  removeFromCart(cartItem:CartItem){
    ...
    console.log(index);
    this.totalAmount -= +cartItem.book.price.value * cartItem.quantity
    this.totalAmount = Math.round(this.totalAmount*100)/100
  ...
  increaseQuantity(cartItem:CartItem){
    ...
    this.totalAmount += cartItem.book.price.value;
    this.totalAmount = Math.round(this.totalAmount*100)/100
  ...
  decreaseQuantity(cartItem:CartItem){
    ...
    this.cartItems[index].quantity-=1;
    this.totalAmount -= (Math.round(cartItem.book.price.value*1000)/1000);
    }
  }

  filterOnTitle(query:string): Book[] {
    let returnArray:Array<Book> = this.books.filter((book)=>book.title.toLowerCase().includes(query.toLowerCase()));
    return returnArray;
  }

  filterOnAuthor(query:string): Book[]{
    let returnArray:Array<Book> = this.books.filter((book)=>book.author.toLowerCase().includes(query.toLowerCase()));
    return returnArray;
  }
  ...
  findElementsByName(){
      ...
      this.displayedBooks = this.books;
      let titleFilteredBooks = this.filterOnTitle(this.searchForm.value?.title);
      console.log(titleFilteredBooks);
      let authorFilteredBooks = this.filterOnAuthor(this.searchForm.value?.title);
      console.log(authorFilteredBooks);
      let arr = [...titleFilteredBooks,...authorFilteredBooks];
